...
module Decidim
  module Referendums
    module Admin
      # A form object used to show the referendum data in the administration
      # panel.
 ...
 validates :decidim_areas_id, presence: true    #aggiunto luca
        ...
        def signature_type_updatable?
          @signature_type_updatable ||= begin
                                          state ||= context.referendum.state
...
#          type.scopes.find_by!(decidim_scopes_id: decidim_scope_id).id
        ...
        def type
          @type ||= type_id ? Decidim::ReferendumsType.find(type_id) : context.referendum.type
        end
      end
    end
  end
