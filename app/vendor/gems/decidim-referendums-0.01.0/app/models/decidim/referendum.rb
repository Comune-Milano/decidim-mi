...
module Decidim
  # The data store for a Referendum in the Decidim::Referendums component.
  class Referendum < ApplicationRecord
    ...
    include Decidim::Referendums::ReferendumSlug
    ...
    has_many :votes,
             ...
             class_name: "Decidim::ReferendumsVote",
             dependent: :destroy,
             inverse_of: :referendum

    has_many :committee_members,
             foreign_key: "decidim_referendums_id",
             class_name: "Decidim::ReferendumsCommitteeMember",
             dependent: :destroy,
             inverse_of: :referendum
    ...
    # Referendums are not intended to have categories.
    has_many :categories,
    ...
    scope :order_by_supports, -> { order(Arel.sql("referendum_votes_count + coalesce(offline_votes, 0) desc")) }
    scope :order_by_most_commented, lambda {
      select("decidim_referendums.*")
          .left_joins(:comments)
          .group("decidim_referendums.id")
    ...
    def self.log_presenter_class_for(_log)
      Decidim::Referendums::AdminLog::ReferendumPresenter
    end

    # PUBLIC
    #
    # Returns true when an referendum has been created by an individual person.
    ...
    # PUBLIC
    #
    # RETURN boolean TRUE when the referendum is open, false in case its
    # not closed.
    def open?
      !closed?
    end
    ...
    # PUBLIC
    #
    # Returns when an referendum is closed. An referendum is closed when
    ...
    # Public: Checks if the organization has given an answer for the referendum.
    #
    # Returns Boolean.
    def answered?
      answered_at.present?
    ...
    # participatory space or assemblies. For referendums it won't be directly
    ...
    # For referendums it won't be directly
    # managed by the user and it will be enabled by default.
    def scopes_enabled
      true
    end

    # Public: Publishes this referendum
    #
    ...
    def publish!
      ...
      update(
          ...
          state: "published",
          signature_start_date: Date.current,
          signature_end_date: Date.current + Decidim::Referendums.default_signature_time_period_length
      )
    end

    #
    # Public: Unpublishes this referendum
    #
    # Returns true if the record was properly saved, false otherwise.
    def unpublish!
    ...
    # Public: Returns the hashtag for the referendum.
    def hashtag
    ...
    def is_offline_signature_date_passed(referendum_id)
      sql = "select now() >= signature_last_day from decidim_referendums where id = #{referendum_id};"
      result = ActiveRecord::Base.connection.select_value(sql)
      return result
    end

    def is_gestione_firme_icona_visibile(user, referendum)
      return true if user.admin?
      if user.role?("initiative_manager")
        sql = "select true from decidim_referendums WHERE id = #{referendum.id} and NOW() > signature_last_day"
        #result = ActiveRecord::Base.connection.select_value(sql)
        #if result.to_s == ""
        # return true
        #end
      else
        sql = "select id from decidim_referendums WHERE id = #{referendum.id} and NOW() BETWEEN signature_end_date AND signature_last_day"
        #result = ActiveRecord::Base.connection.select_value(sql)
        #if result.to_s == "true"
        #  return true
        #end
    ...
    def check_pdf(referendum_id)
      sql = "SELECT attachment FROM public.pdf_signeds where component_id = #{referendum_id} and component_type = 'referendum';"
      records_array = ActiveRecord::Base.connection.select_all(sql)
      true if records_array.present?
    end

    def check_csv(referendum_id)
      sql = "SELECT id FROM public.decidim_referendums_csv_signature_data where referendums_id = #{referendum_id};"
      records_array = ActiveRecord::Base.connection.select_all(sql)
      true if records_array.present?
    end
    ...
    def minimum_committee_members
      type.minimum_committee_members || Decidim::Referendums.minimum_committee_members
    end
    ...
    # PUBLIC
    #
    # Checks if the type the referendum belongs to enables SMS code
    ...
    def signature_type_allowed
      ...
      errors.add(:signature_type, :invalid) if type.allowed_signature_types_for_referendums.exclude?(signature_type)
    end

    def notify_state_change
      return unless saved_change_to_state?

      notifier = Decidim::Referendums::StatusChangeNotifier.new(referendum: self)
      notifier.notify
    end
