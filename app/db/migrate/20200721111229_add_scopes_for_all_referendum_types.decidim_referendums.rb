...
# This migration comes from decidim_referendums (originally 20171017091734)

class AddScopesForAllReferendumTypes < ActiveRecord::Migration[5.1]
  ...
  def up
    ...
    Organization.find_each do |organization|
      ReferendumsType.where(organization: organization).find_each do |type|
        organization.scopes.each do |scope|
          ReferendumsTypeScope.create(
            decidim_referendums_types_id: type.id,
  ...
  def down
    Decidim::ReferendumsTypeScope.destroy_all
  end
end
