...
render :json => settings
  ...
  def ssologin
    ...
    logger.warn "*** BEGIN 2 RAW ALL REQUEST HEADERS ***"
	...
	if header[0].include? "HTTP"
     ...
     #logger.warn "+-+-+-+-+-+ " + self.request.headers['HTTP_REFERER']i
     logger.info "+++++++++++++++++ " + session[:session_id].to_json
    # if (session[:session_id])
#	     logger.info "----------------- " + session[:session_id].to_json

   # else 
  #   if (session[:session_id].to_json.empty?)
#	    logger.info "PAPERINO " + self.request.headers['HTTP_REFERER'].to_s
#	    if(self.request.headers['HTTP_REFERER'].to_s == 'https://portal-coll.ecaas.comune.milano.it/'  )
#		    logger.info "PLUTO "
#	     redirect_to ('/users/auth/saml')
#	    end
#	end
    # logger.warn "+-+-+-+-+-+ " + self.request.headers['HTTP_REFERER'] 
...
#     logger.warn "\n\n\nPRIMA\n\n\n"
     if(self.request.headers['HTTP_CDM_CF'])
       ...
       realm = Rails.configuration.token_autenticazione_servizi
...
=end
  ...
  def self.wsresidenza?(cf,matricola,realm)
        ...
        codice_status_anagrafico = response.body.to_hash[:get_dati_puntuali_xml_response][:get_dati_puntuali_xml_result][:dati_individuo][:codice_status_anagrafico].to_s
               Rails.logger.info "codice_status_anagrafico " + codice_status_anagrafico.to_s
